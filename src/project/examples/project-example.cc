...
VanetRoutingExperiment::ConfigureApplications ()
{
  ...
  // This function sets up AODV routing in the network however I have disabled
  // the on off application
  ...
  for (int i=0; i < evilNodes; i++) 
  {
    ApplicationContainer temp = evilAppHelper.Install(m_adhocTxNodes.Get(i));
    temp.Start(Seconds(0));
    temp.Stop(Seconds(m_TotalSimTime));
  }

  for (uint32_t i=evilNodes-1; i < m_nNodes; i++)
  {
    ApplicationContainer temp = goodAppHelper.Install(m_adhocTxNodes.Get(i));
    temp.Start(Seconds(0));
    temp.Stop(Seconds(m_TotalSimTime));
  }
  ...
  // app.Start(Seconds(0));
  // app.Stop(Seconds(m_TotalSimTime));
...
VanetRoutingExperiment::SetupScenario ()
{
  ...
  // certain par.meters may be further overridden
