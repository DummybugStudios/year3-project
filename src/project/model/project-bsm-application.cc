...
NS_LOG_COMPONENT_DEFINE ("ProjectBsmApplication");

namespace ns3 {

// (Arbitrary) port for establishing socket to transmit WAVE BSMs
    int ProjectBsmApplication::wavePort = 9080;

    NS_OBJECT_ENSURE_REGISTERED (ProjectBsmApplication);

    TypeId
    ProjectBsmApplication::GetTypeId(void) {
    ...
    ProjectBsmApplication::ProjectBsmApplication()
            : m_waveBsmStats(0),
              m_txSafetyRangesSq(),
    ...
    void
    ProjectBsmApplication::DoDispose(void) {
        NS_LOG_FUNCTION(this);
...
// Application Methods
    void ProjectBsmApplication::StartApplication() // Called at time specified by Start
    {
        NS_LOG_FUNCTION(this);
        ...
        recvSink->SetRecvCallback(MakeCallback(&ProjectBsmApplication::ReceiveWavePacket, this));
    ...
    void
    ProjectBsmApplication::Setup(Ipv4InterfaceContainer &i,
                                 int nodeId,
                                 Time totalTime,
    ...
    void
    ProjectBsmApplication::GenerateWaveTraffic(Ptr <Socket> socket, uint32_t pktSize,
        ...
        if (pktCount > 0) {
            // for now, we cannot tell if eachnode has
    ...
    void ProjectBsmApplication::ReceiveWavePacket(Ptr <Socket> socket) {
        NS_LOG_FUNCTION(this);
    ...
    void ProjectBsmApplication::HandleReceivedBsmPacket(Ptr <Node> txNode,
                                                        Ptr <Node> rxNode) {
        NS_LOG_FUNCTION(this);
    ...
    int64_t
    ProjectBsmApplication::AssignStreams(int64_t streamIndex) {
        NS_LOG_FUNCTION(this);
    ...
    Ptr <Node>
    ProjectBsmApplication::GetNode(int id) {
        NS_LOG_FUNCTION(this);
    ...
    Ptr <NetDevice>
    ProjectBsmApplication::GetNetDevice(int id) {
        NS_LOG_FUNCTION(this);
